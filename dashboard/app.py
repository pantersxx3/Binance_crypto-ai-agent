"""
dashboard/app.py - Backend con soporte completo para sesiones y modelos
"""
import asyncio
import json
import os
...
async def get_session_detail(session_id: str):
    try:
        ...
        conn.close()
        return {"session": dict(session_row), "trades": [dict(trade) for trade in trades]}
    except Exception as e:
    ...
    if MODELS_DIR.exists():
        for db_file in MODELS_DIR.glob("*.db"):
            ...
            try:
                ...
                logger.debug(f"Error reading model DB: {e}")
    return {"models": sorted(models, key=lambda x: x.get("win_rate", 0), reverse=True)}
...
async def get_model_stats(model_name: str):
    ...
    try:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table'")
        tables = [row[0] for row in cursor.fetchall()]
        stats = {"summary": {"win_rate": 0, "total_trades": 0, "total_pnl_pct": 0, "avg_confidence": 0, "total_decisions": 0}, "recent_decisions": []}
        ...
        if 'outcomes' in tables:
            cursor.execute('''
                ...
                FROM outcomes o INNER JOIN decisions d ON o.decision_id = d.id
            ''')
            row = cursor.fetchone()
            if row and row[0] > 0:
...
async def get_model_trades(model_name: str, limit: int = 20):
    try:
        safe_name = model_name.replace(":", "").replace("%3A", "")
        db_path = MODELS_DIR / f"{safe_name}.db"
        if not db_path.exists():
        ...
        cursor.execute('''
            SELECT d.timestamp, d.pair, d.direction, d.confidence, 
                   ...
                   d.stop_loss_price, d.take_profit_price
            FROM decisions d
        ...
        trades = []
        for row in cursor.fetchall():
            status = 'OPEN' if row[7] is None else ('WIN' if row[7] == 1 else 'LOSS')
            trades.append({
                "time": row[0],
                "pair": row[1],
                "side": row[2],
    ...
    try:
        ...
        cursor.execute('''
            SELECT d.id, d.timestamp, d.pair, d.direction, d.confidence,
                   ...
                   d.market_regime, d.trend_strength
            FROM decisions d LEFT JOIN outcomes o ON d.id = o.decision_id
